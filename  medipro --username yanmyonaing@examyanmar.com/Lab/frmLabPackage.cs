...
namespace Lab
{
    public partial class frmLabPackage : Form
    {
        clsSQL SqlDb = new clsSQL();
        DataRow drLab;
        int curIndex;

        public frmLabPackage()
        {
            InitializeComponent();
        }
        ...
        {
            DataSet dsLab = SqlDb.GetDataSet("SELECT labPackPK, labPackName, isActive FROM tblLabPackage WHERE isDelete = 0 and labPackName like '%"+txtSearch.Text+"%'");
            grdLaboratory.DataSource = dsLab.Tables[0];
