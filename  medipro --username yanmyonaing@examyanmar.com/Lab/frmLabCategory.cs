...
namespace Lab
{
    public partial class frmLabCategory : Form
    {
        clsSQL SqlDb = new clsSQL();
        DataRow dr;
        int curIndex;

        public frmLabCategory()
        {
            InitializeComponent();
        }
        ...
        private void frmLabCat_Load(object sender, EventArgs e)
        {
        ...
        {
            DataSet ds = SqlDb.GetDataSet("SELECT labCatPK, labCatName, isActive FROM tblLabCat WHERE isDelete = 0");
        ...
        private void Edit()
        {
            if (SqlDb.IsAllow(AppVariable.CURRENT_SUB_MENU, AppVariable.CURRENT_USER_LEVEL_ID.ToString(), "IsEdit") == true)
            {
                ...
                frmLabCatDetail detailForm = new frmLabCatDetail();
        ...
        {
            DataSet ds = SqlDb.GetDataSet("SELECT labCatPK, labCatName, isActive FROM tblLabCat WHERE isDelete = 0 and labCatName like '%"+txtSearch.Text+"%'");
            grd.DataSource = ds.Tables[0];
