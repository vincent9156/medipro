...
namespace Lab
{
    public partial class frmLabTest : Form
    {
        ...
        private void frmLabSubTest_Load(object sender, EventArgs e)
        {
        ...
        {
            DataSet ds = SqlDb.GetDataSet("SELECT labTestPK,labTestCode, labTestName,labTestDescription, isActive FROM tblLabTest WHERE isDelete = 0");
        ...
        private void Edit()
        {
            if (SqlDb.IsAllow(AppVariable.CURRENT_SUB_MENU, AppVariable.CURRENT_USER_LEVEL_ID.ToString(), "IsEdit") == true)
            {
                ...
                frmLabTestDetail detailForm = new frmLabTestDetail();
        ...
        {
            DataSet ds = SqlDb.GetDataSet("SELECT labTestPK,labTestCode, labTestName,labTestDescription, isActive FROM tblLabTest WHERE isDelete = 0 and labTestName like '%" + txtSearch.Text + "%'");
            grd.DataSource = ds.Tables[0];
