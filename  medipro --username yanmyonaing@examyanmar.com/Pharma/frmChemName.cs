...
namespace Pharma
{
    public partial class frmChemName : Form
    {
        ...
        private void Edit()
        {
            if (SqlDb.IsAllow(AppVariable.CURRENT_SUB_MENU, AppVariable.CURRENT_USER_LEVEL_ID.ToString(), "IsEdit") == true)
            {
                ...
                frmChemNameDetail detailForm = new frmChemNameDetail();
        ...
        {
            DataSet ds = SqlDb.GetDataSet("SELECT chemNamePK, chemName, isActive FROM tblChemName WHERE isDelete = 0 and chemName like '%"+txtSearch.Text+"%'");
            grd.DataSource = ds.Tables[0];
